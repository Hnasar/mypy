...
class State:
    ...
    def _maybe_inject_synthetic_getattr_for_typed_subpackage(self) -> None:
        ...
        if self.id not in self.manager.find_module_cache.ns_ancestors:
            return
        # A directory path means the ns_ancestors fallback in `_find_module`
        # bound this State to the package directory and `__init__.py` was never
        # read. The normal find path (and `--follow-untyped-imports`) instead
        # bind `path` to the `__init__.py` file itself, in which case mypy
        # parses it and we don't want to interfere.
        if not self.path or not self.manager.fscache.isdir(self.path):
            return
        # Skip true namespace packages (no __init__.py).
        if not (
        ...
        # Hidden + non-public: keep the synthetic invisible to direct
        # `pkg.__getattr__` and `from pkg import __getattr__` accesses
        # (those fall through to `types.ModuleType.__getattr__` as they
        # would for any plain module). mypy's internal __getattr__-fallback
        # path reads `tree.names["__getattr__"]` directly and bypasses
        # both flags, so the helper is still consulted for `pkg.X` lookups.
